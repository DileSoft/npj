...
if (!class_exists("typografica"))
{
    class typografica
    {
      ...
      var $phonemasks = array(
                            ...
                            array("<nobr>\\1&nbsp;\\2&minus;\\3&minus;\\4</nobr>",
                                  "<nobr>\\1&nbsp;\\2&minus;\\3&minus;\\4</nobr>",
                                  "<nobr>\\1&nbsp;\\2&minus;\\3</nobr>",
                                  "<nobr>\\1&nbsp;\\2&minus;\\3</nobr>",
                                  "<nobr>\\1&minus;\\2&minus;\\3</nobr>",
                                  "<nobr>\\1&minus;\\2&minus;\\3</nobr>",
                                  "<nobr>\\1&minus;\\2&minus;\\3</nobr>",
                                  "<nobr>\\1&minus;\\2</nobr>"
                                  )
      ...
      var $glueright = array( "руб\.", "коп\.", "у\.е\.", "мин\.", "кг", "кгс" );
      ...
      function typografica ( &$wakka )
      {
        $this->wakka = &$wakka;
        $this->Indent1 = "<img src=".$wakka->images_dir.$this->Indent1;
        $this->Indent2 = "<img src=".$wakka->images_dir.$this->Indent2;
      }
      ...
      function replaceSpecials( $data )
      {
        ...
        if ($this->settings["dash"])
          $data = preg_replace( "/(\s|;)\-(\s)/i", "\\1".$this->mdash."\\2", $data );
        // 3a. тире длинное
        if ($this->settings["emdash"])
          $data = preg_replace( "/(\s|;)\-\-(\s)/i", "\\1".$this->mdash."\\2", $data );
              ...
              // 4a. (r)
              if ($this->settings["(r)"])
                $data = preg_replace( "/\(r\)/i", "<sup>&reg;</sup>", $data );
              // 4b. (tm)
              if ($this->settings["(tm)"])
                $data = preg_replace( "/\(tm\)|\(тм\)/i", "&trade;", $data );
              // 4c. (p)
              if ($this->settings["(p)"])
                $data = preg_replace( "/\(p\)/i", "&sect;", $data );

        // 5. +/-
        if ($this->settings["+-"])
          $data = preg_replace( "/\+\-/i", "&plusmn;", $data );
        // 5a. 12^C
        if ($this->settings["degrees"])
        {
          $data = preg_replace( "/-([0-9])+\^([FCС])/", "&minus;\\1&deg;\\2", $data );
          $data = preg_replace( "/\+([0-9])+\^([FCС])/", "+\\1&deg;\\2", $data );
          $data = preg_replace( "/\^([FCС])/", "&deg;\\1", $data );
        }
        // 5б. м^3
        if ($this->settings["degrees"])
        {
          $data = preg_replace( "/\^([0-9])/", "<sup>\\1</sup>", $data );
        }
