...
class MagpiePlus {
  ...
  var $items			= array();	// collection of parsed items
	...
	var $feed_type;
	var $feed_version;
	var $default_encoding = "windows-1251";
...
//    require_once(dirname(__FILE__).'/HTMLSax.php');
    $parser=& new XML_HTMLSax();
		...
		// decode Russian encoding to windows-1251
    preg_match("/^<\?xml[^>]*encoding=[\'\"]?(.*?)[\'\"]?(\s|\?)/i", trim($source), $matches);
    if (isset($matches[1]))
  	  $this->channel["encoding"] = $matches[1];
  	else
      $this->channel["encoding"] = $this->default_encoding;
		
		if ($this->channel["encoding"]!="windows-1251")
		{
		  if (stristr($this->channel["encoding"], "koi"))
		    $source = convert_cyr_string($source, "k", "w");

		  if (strtolower($this->channel["encoding"])=="utf-8")
		    $source = $this->UtfDecode($source);
		}
    ...
    // Set the handlers
    $parser->set_element_handler('feed_start_element', 'feed_end_element');
    $parser->set_data_handler('feed_data');
    //$parser->set_pi_handler('feed_pi');
    $parser->set_escape_handler('feed_cdata');
    $status  = $parser->parse($source);
...
*/
	...
	function feed_data ($p, $text) 
	{
	//called when no CDATA here
	//seems to be that we need to Decode Entities Here
		if ($this->feed_type == ATOM and $this->incontent) 
		{
			$this->append_content( $this->RemoveHtmlEntities($text) );
		}
		else 
		{
			$current_el = join('_', array_reverse($this->stack));
			$this->append($current_el, $this->RemoveHtmlEntities($text));
		}
	}

	function feed_cdata ($p, $text) 
	{
	//called when CDATA here
		if ($this->feed_type == ATOM and $this->incontent) 
		{
	...
	function feed_pi (&$parser,$name,$attrs) 
	{
	  if ($name=="xml") 
	  {
      preg_match("/encoding=[\'\"]?(.*?)[\'\"]?(\s|\?)/i", $attrs, $matches);
      if (isset($matches[1]))
    	  $this->channel["encoding"] = $matches[1];
    	else
        $this->channel["encoding"] = $this->default_encoding;
	...
	function normalize () {
		...
		if ( $this->is_atom() ) 
		{
			...
			for ( $i = 0; $i < count($this->items); $i++) 
			{
				...
				{
					$item['content'] = $item['atom_content'];
					unset($item['atom_content']);
				}
				// === date
				if ( isset($item['updated']))
				{
					$item['date'] = $this->ConvertDateAtom($item['updated']);
					unset($item['updated']);
				}
				if ( !isset($item['date']) && isset($item['created']))
				{
					$item['date'] = $this->ConvertDateAtom($item['created']);
					unset($item['created']);
				}
				if ( !isset($item['date']) && isset($item['published']))
				{
					$item['date'] = $this->ConvertDateAtom($item['published']);
					unset($item['published']);
				}
				if ( !isset($item['date']) && isset($item['issued']))
				{
					$item['date'] = $this->ConvertDateAtom($item['issued']);
					unset($item['issued']);
				}
				// === author
				if ( isset($item['author_name']))
				{
					$item['author'] = $item['author_name'].($item['author_email']?" ".$item['author_email']:"");
		...
		{
			$item = $this->items[$i];
    	if (is_array($item['content']) && isset($item['content']['encoded']))
    	{
        $item['content'] = $item['content']['encoded'];
    	}
    	if (isset($item['pubdate']))
    	{
        $item['date'] = $this->ConvertDateRSS($item['pubdate']);
        unset($item['pubdate']);
    	}
    	if (!isset($item['description']) && isset($item['dc']['description']))
    	{
        $item['description'] = $item['dc']['description'];
        unset($item['dc']['description']);
    	}
    	if (!isset($item['date']) && isset($item['dc']['date']))
    	{
        $item['date'] = $this->ConvertDateAtom($item['dc']['date']);
        unset($item['dc']['date']);
    	}
      $this->items[$i] = $item;
    }
...
*/
	...
	function ConvertDateRSS ($datetime)
	{
	  //Thu, 08 Jul 1999 07:00:00 GMT
	  //Sat, 07 Sep 2002 00:00:01 GMT
	  //Sat, 07 Sep 99 00:00:01 GMT
	...
	function ConvertDateAtom ($datetime)
	{
    $datetime = preg_replace("/(\d\d\d\d-\d\d-\d\d)T/", "$1 ", $datetime);
    $datetime = preg_replace("/(\d\d:\d\d:\d\d)((\+|-)\d\d):(\d\d)/", "$1 $2$4", $datetime);
    $timestamp = strtotime($datetime);
    return date("Y-m-d H:i:s", $timestamp);
	}
