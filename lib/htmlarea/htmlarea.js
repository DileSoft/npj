...
HTMLArea.prototype.generate = function () {
  ...
  if (height < 0 || height == 0) {
    iframe.style.height = this.config.JsHeight;
  }
  else
  ...
  textarea.style.width = "100%";//iframe.style.width;
  textarea.style.height = iframe.style.height ? iframe.style.height : 400;
  ...
  function initIframe() {
    ...
    if (!editor.config.fullPage) {
      ...
      /* Здесь было формирования HTML с подключением стилей.
      * Всё это убрано, поскольку шаблон страницы формируется прямо в textarea.     
      * Стили добавляются туда всырую.
      */
      
      doc.open();
      doc.write(editor._textArea.value);
      doc.close();
...
HTMLArea.prototype._editorEvent = function(ev) {
  var editor = this;
  if (HTMLArea.is_ie && ev.type == "keypress")  this._criticalForm();
...
HTMLArea.prototype._criticalForm = function() {
  var doc = this._doc;
  cf_modified = true;
  doc.body.style.backgroundColor="#fffef5";
};
