...
($this->object->GetConfigValue("disable_npjlinks")==1?
 "":"(~?)".ALPHANUM."+\@".ALPHA."*(?!".ALPHANUM."*\.".ALPHANUM."+)(\:".ALPHANUM."*)?|".ALPHANUM_L."+\:\:".ALPHANUM."+|").
"\n)/sm";
...
")/sm";
  ...
  function WackoPreprocess($things)
  {
    ...
    else if (preg_match("/^\%\%(.*)\%\%$/s", $thing, $matches))
    {
      ...
      // kuso to kukutz: надо написать проверку на наличие такого метода _Wrapper,
      //                 я пока "значительно упростил"
      if ($params["wrapper"])
      {
        $res = $wacko->_Wrapper(trim($res), $params);
      }
  ...
  function WackoCallback($things)
  {
    ...
    $this->page_id = $wacko->data["record_id"];
    ...
    // npj links
             ...
             (preg_match("/^(~?)(".ALPHANUM."+\@".ALPHA."*(\:".ALPHANUM."*)?|".ALPHANUM_L."+\:\:".ALPHANUM."+)$/s", $thing, $matches)))
    {
      if ($matches[1]=="~")
       return $matches[2];
      return $wacko->PreLink($thing); 
